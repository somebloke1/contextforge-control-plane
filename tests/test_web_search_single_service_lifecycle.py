...
class WebSearchSingleServiceLifecycleTests(unittest.TestCase):
    ...
    def approve_and_apply(self, root: Path, *, client_type: str) -> dict:
        proposal = helper.propose_project_init(
            ...
            selected_services=["web-search:credential_scoped"],
        )
        self.assertEqual([], proposal["skipped_services"])
        selected = proposal["selected_services"][0]
        self.assertEqual("web-search:credential_scoped", selected["service_binding"])
        self.assertEqual("credential_scoped", selected["instantiation_class"])
        self.assertEqual("shared_canonical", selected["activation_class"])
        self.assertEqual("credential-scoped hosted binding", selected["scope_label"])
    ...
    def assert_web_search_state(self, state: dict, *, client_type: str) -> None:
        self.assertIn("web-search:credential_scoped", state["services"])
        service = state["services"]["web-search:credential_scoped"]
        self.assertEqual("web-search", service["service_family"])
        self.assertEqual("web-search:credential_scoped", service["service_binding"])
        self.assertEqual("credential_scoped", service["instantiation_class"])
        self.assertEqual("server-instances/web-search", service["backend_instance"])
        self.assertEqual("web_search_server", service["virtual_server"])
        ...
        self.assertEqual(["web-search", "fetch-content"], policy["safe_operations"])
    ...
    def test_web_search_pi_lifecycle_writes_only_project_state_and_stops_for_reload(self) -> None:
        root = self.make_workspace_root("cf267-pi-test-")
        ...
        self.assert_web_search_state(state, client_type="pi")
        self.assertEqual(
            "shim_activation_planned",
            state["services"]["web-search:credential_scoped"]["target_clients"]["pi"]["status"],
        )

    def test_web_search_opencode_lifecycle_writes_project_config_state_and_stops_for_new_session(self) -> None:
        root = self.make_workspace_root("cf267-opencode-test-")
        ...
        self.assertIn("web_search", opencode_config["mcp"])
        self.assertEqual("local", opencode_config["mcp"]["web_search"]["type"])
        command = opencode_config["mcp"]["web_search"]["command"]
        self.assertTrue(any(str(arg).endswith("contextforge_mcp_wrapper.py") for arg in command))
        self.assertIn("web_search_server", command)
        ...
        for token_name in (
            ...
            self.assertNotIn(token_name, serialized)
        self.assertIsNotNone(state)
        assert state is not None
        self.assert_web_search_state(state, client_type="opencode")
        self.assertEqual(
            "project_local_opencode_config_planned",
            state["services"]["web-search:credential_scoped"]["target_clients"]["opencode"]["status"],
        )
