...
class PlaywrightSingleServiceLifecycleTests(unittest.TestCase):
    ...
    def approve_and_apply(self, root: Path, *, client_type: str) -> dict:
        proposal = helper.propose_project_init(
            ...
            selected_services=["playwright:session_scoped"],
        )
        self.assertEqual([], proposal["skipped_services"])
        selected = proposal["selected_services"][0]
        self.assertEqual("playwright:session_scoped", selected["service_binding"])
        self.assertEqual("session_scoped", selected["instantiation_class"])
        self.assertFalse(selected["bridge"]["needed"])
    ...
    def assert_playwright_state(self, state: dict, *, client_type: str) -> None:
        self.assertIn("playwright:session_scoped", state["services"])
        service = state["services"]["playwright:session_scoped"]
        self.assertEqual("playwright", service["service_family"])
        self.assertEqual("playwright:session_scoped", service["service_binding"])
        self.assertEqual("session_scoped", service["instantiation_class"])
        self.assertEqual("server-instances/playwright", service["backend_instance"])
        self.assertEqual("playwright_server", service["virtual_server"])
        ...
        self.assertEqual("read_only_if_semantics_allow", policy["mode"])
        self.assertEqual(["list-tools", "browser-snapshot"], policy["safe_operations"])
    ...
    def test_playwright_pi_lifecycle_writes_only_project_state_and_stops_for_reload(self) -> None:
        root = self.make_workspace_root("cf265-pi-test-")
        ...
        self.assert_playwright_state(state, client_type="pi")
        self.assertEqual(
            "shim_activation_planned",
            state["services"]["playwright:session_scoped"]["target_clients"]["pi"]["status"],
        )

    def test_playwright_opencode_lifecycle_writes_project_config_state_and_stops_for_new_session(self) -> None:
        root = self.make_workspace_root("cf265-opencode-test-")
        ...
        self.assertIn("playwright", opencode_config["mcp"])
        self.assertEqual("local", opencode_config["mcp"]["playwright"]["type"])
        command = opencode_config["mcp"]["playwright"]["command"]
        self.assertTrue(any(str(arg).endswith("contextforge_mcp_wrapper.py") for arg in command))
        self.assertIn("playwright_server", command)
        self.assertIsNotNone(state)
        assert state is not None
        self.assert_playwright_state(state, client_type="opencode")
        self.assertEqual(
            "project_local_opencode_config_planned",
            state["services"]["playwright:session_scoped"]["target_clients"]["opencode"]["status"],
        )
