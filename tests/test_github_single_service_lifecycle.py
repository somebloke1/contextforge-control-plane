...
class GitHubSingleServiceLifecycleTests(unittest.TestCase):
    ...
    def approve_and_apply(self, root: Path, *, client_type: str) -> dict:
        proposal = helper.propose_project_init(
            ...
            selected_services=["github:canonical"],
        )
        self.assertEqual([], proposal["skipped_services"])
        selected = proposal["selected_services"][0]
        self.assertEqual("github:canonical", selected["service_binding"])
    ...
    def assert_github_state(self, state: dict, *, client_type: str) -> None:
        self.assertIn("github:canonical", state["services"])
        service = state["services"]["github:canonical"]
        self.assertEqual("github", service["service_family"])
        self.assertEqual("github:canonical", service["service_binding"])
        self.assertEqual("credential_scoped", service["instantiation_class"])
        self.assertEqual("server-instances/github", service["backend_instance"])
        self.assertEqual("github_server", service["virtual_server"])
        ...
        self.assertEqual("read_only_if_credentials_available", policy["mode"])
        self.assertEqual(["search-repositories", "list-issues", "get-file-contents"], policy["safe_operations"])
    ...
    def test_github_pi_lifecycle_writes_only_project_state_and_stops_for_reload(self) -> None:
        root = self.make_workspace_root("cf262-pi-test-")
        ...
        self.assert_github_state(state, client_type="pi")
        self.assertEqual(
            "shim_activation_planned",
            state["services"]["github:canonical"]["target_clients"]["pi"]["status"],
        )

    def test_github_opencode_lifecycle_writes_project_config_state_and_stops_for_new_session(self) -> None:
        root = self.make_workspace_root("cf262-opencode-test-")
        ...
        self.assertIn("github", opencode_config["mcp"])
        self.assertEqual("local", opencode_config["mcp"]["github"]["type"])
        command = opencode_config["mcp"]["github"]["command"]
        self.assertTrue(any(str(arg).endswith("contextforge_mcp_wrapper.py") for arg in command))
        self.assertIn("github_server", command)
        serialized = json.dumps(opencode_config)
        self.assertNotIn("GITHUB_PERSONAL_ACCESS_TOKEN", serialized)
        self.assertNotIn("ghp_", serialized)
        self.assertNotIn("github_pat_", serialized)
        self.assertIsNotNone(state)
        assert state is not None
        self.assert_github_state(state, client_type="opencode")
        self.assertEqual(
            "project_local_opencode_config_planned",
            state["services"]["github:canonical"]["target_clients"]["opencode"]["status"],
        )
