...
class ServiceOnboardingHelperDocTests(unittest.TestCase):
    ...
    def test_doc_records_research_packet_readback_contract(self) -> None:
        for phrase in [
            ...
            "seed lead is all it knows at handoff time",
            "executable or bridge entrypoints",
            "tool contracts",
            "configuration",
            "auth boundary",
    ...
    def test_doc_records_ordered_onboarding_development_foil_ladder(self) -> None:
        ...
        self.assertIn("fresh agent with no service-specific project history", self.doc)
        self.assertIn("only the source", self.doc)
