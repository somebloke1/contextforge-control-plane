...
class ContextForgeDevelopmentPathIndexTests(unittest.TestCase):
    ...
    def test_project_coordination_skill_defines_minimal_board_practice(self) -> None:
        for text in [
            ...
            "If a single-select enum value is not defined here",
            ...
            "Do not set\n`Squad 1`, `Squad 2`, or `Squad 3`",
            "Workflow Knowledge Persistence",
            "Persist the workflow automation map in this skill",
            "GraphQL Budget Discipline",
            "Cache the Project ID, item IDs, field IDs",
            "`gh project item-list --query`",
            "Use REST-backed issue/PR reads",
            "Use native REST issue/PR mutations",
            "Compute updates locally and skip no-op mutations",
            "direct\n  `gh api graphql` mutation batch",
            "not a transaction boundary",
            "Avoid concurrent Project mutation runs",
            "secondary\n  limits",
            "Avoid per-item readback",
            "follow `pageInfo`\n  cursors",
            "content can be `REDACTED`",
            "cannot be added and field-updated in the same API\n  call",
            "Prefer rate-limit response headers",
            "GraphQL remaining budget is low",
            "Issue #166 owns the future helper implementation",
            "read Project #6",
            "workflow contract",
