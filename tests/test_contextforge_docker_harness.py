...
class ContextForgeDockerHarnessTests(unittest.TestCase):
    ...
    def test_pi_interactive_launcher_supports_manual_tui_validation(self) -> None:
        launcher = (ROOT / "docker/client-harness/scripts/start-pi-contextforge-interactive.sh").read_text(encoding="utf-8")
        readme = (ROOT / "docker/client-harness/README.md").read_text(encoding="utf-8")

        self.assertIn("CONTEXTFORGE_PI_INTERACTIVE_CONTAINER:-contextforge-pi-interactive", launcher)
        self.assertIn("docker attach \"${SESSION_NAME}\"", launcher)
        self.assertIn("docker compose -f compose.yml run --rm --no-deps", launcher)
        self.assertIn("--name \"${SESSION_NAME}\"", launcher)
        self.assertIn("--interactive", launcher)
        self.assertIn("-v \"${REPO_ROOT}:/repo:ro\"", launcher)
        self.assertNotIn("-v \"${REPO_ROOT}:/workspace:ro\"", launcher)
        self.assertIn("workspace/.project", launcher)
        self.assertIn("context_forge_state.json", launcher)
        self.assertIn("mentality:dev_docker", launcher)
        self.assertIn("target_clients", launcher)
        ...
        self.assertNotIn("exec docker compose -f compose.yml run", launcher)
        self.assertIn("pi /config/pi/start-contextforge-baseline.sh \"$@\"", launcher)
        self.assertIn("Do not pass -p, --no-session, --no-tools, or --no-context-files", launcher)
        self.assertNotIn("/home/dgk/.pi", launcher)
        self.assertNotIn("/home/agent/.pi/agent/extensions", launcher)

        self.assertIn("Interactive Pi TUI Session", readme)
        self.assertIn("Issue #198 requires a multi-turn Pi session", readme)
        self.assertIn("scripts/start-pi-contextforge-interactive.sh", readme)
        self.assertIn("scripts/start-pi-contextforge-interactive.sh --attach", readme)
        self.assertIn("docker attach contextforge-pi-interactive", readme)
        self.assertIn("Do not pass `-p`, `--no-session`, `--no-tools`, or `--no-context-files`", readme)
        self.assertIn("ask Pi to use `cf_contextforge_pi_readback`", readme)
        self.assertIn("mentality:dev_docker", readme)
        self.assertIn("http://host.docker.internal:4445", readme)
        self.assertIn("The plain baseline launcher", readme)
        self.assertIn("does not install or reload the host user-global Pi extension", readme)
