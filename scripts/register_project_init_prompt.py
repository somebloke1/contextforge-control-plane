...
5. After approved apply, if client_reload_requirement that blocks validation appears, ask only for the explicit client reload or new-session action first and stop. Say that after reload or new session, resume project init and choose 1 or reply "validate" to run validation; choose 2 or reply "skip validation". On resume, first call cf_project_init_record_client_reload with client_type={{ target_client }}; if OpenCode and record_project_init_client_reload is visible, prefer that tool with client_type=opencode. If the resumed message already chose validation or skip, pass validation mode validate_now or presume_working to the reload call and honor that choice instead of asking again. For validation, call the selected probe, then call cf_project_init_record_validation with a top-level result object keyed by service binding. Each entry needs target_client, tool_name, safe_probe_id, proof_kind, safe_probe_result, and verification_trace_refs. Never call validate_now with missing, empty, or invented validation_results. If the probe tool is unavailable, say validation is unavailable and stop. If skipped, record presume_working. Otherwise ask only: "Choose 1 to validate now, or choose 2 to skip validation and record the services as presumed working without marking them verified."

Client localization: Codex uses .codex/config.toml; Codex launches configured MCP servers and exposes their tools when a session starts; /mcp is a status view, not an in-place MCP tool reload; start a new Codex session from the project root before target-client-visible validation. Gemini uses .gemini/settings.json; Gemini's user-global ContextForge hook injects this project-init guidance through SessionStart/BeforeAgent additionalContext; Gemini CLI launches configured MCP servers and exposes their tools when a session starts; start a new Gemini CLI session from the project root before target-client-visible validation. OpenCode uses project-local opencode.json for selected MCP service bindings; OpenCode's user-home ContextForge plugin uses the messages transform hook as the first-prompt trigger and does not rely on experimental.chat.system.transform as the primary init mechanism; OpenCode discovers configured MCP servers and loads user-home plugins when a session starts; start a new OpenCode session from the project root before target-client-visible validation. Pi uses the global extension framework: input-triggered hidden message, cf_project_init_prompt and cf_contextforge_pi_readback are diagnostic only, do not print or summarize raw cf_contextforge_pi_readback JSON, do not dump readback data into chat, use pi.registerTool(), and the Pi agent must issue /reload before validation after an approved global extension install or upgrade.

Validation must be target-client-visible and non-destructive. Use safe read/list/search probes only; otherwise record skipped with reason. Before cf_project_init_record_validation(validate_now), send a top-level result object keyed by service binding. Each validated entry must include status, target_client_visible, proof_kind, safe_probe_result, safe_probe_id, target_client, and verification_trace_refs; do not nest under {"services": ...}. For Pi, use cf_contextforge_pi_validate for validate-now, not cf_contextforge_pi_readback. For context7 validation, call context7_context7-local-resolve-library-id or a visible equivalent before recording validation. Do not call unlisted or unavailable validation tool names. If a validation tool call is missing, not found, unavailable, or returns an error, validation did not pass. An asserted JSON proof without an observed target-client tool call also does not pass. Backend-only ContextForge health and "tool exists" availability are not enough; direct curl checks and invented validation_results are not enough. Empty validation_results are not enough.
