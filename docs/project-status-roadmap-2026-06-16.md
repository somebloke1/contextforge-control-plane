...
- Merged PR
  ...
  and no clean-root project trust or hook-state entries yet.
...
| `codex/issue-31-pending-restart-verification` | [#31](https://github.com/somebloke1/contextforge-control-plane/issues/31) | Runtime verification after the approved write-only user-global Codex config/trust migration. Disk now points the active global helper and global project-init hook commands at the clean root, clean-root project trust exists, and legacy trust/hook-state are preserved by policy. The migration is `pending_restart` until a separately approved Codex restart or fresh-session validation proves live runtime state. | post-write planner readback; direct `~/.codex/config.toml` readback; global and clean-project `codex mcp list --json`; later restart/fresh-session MCP and hook trust readback |
...
7. Continue issue #15 dirty-checkout retirement before starting new
   ...
   hook-command migration. On 2026-06-16, the user approved and the operator
   applied the write-only user-global Codex config/trust migration with #29/#30
   tooling, preserving legacy trust and hook-state, creating backup
   `/home/dgk/.codex/config.toml.contextforge-backup-20260616T221842Z`, and
   leaving the migration in `pending_restart`. No hook execution, hook
   approval/toggle, Codex reload/restart, prompt/resource upsert,
   ...
   Choose the next separately approved operator-path step: perform #31 runtime
   `pending_restart` verification after an explicit Codex restart or fresh
   session, perform systemd/Serena reload/readback, perform ContextForge
   registration readback, or decide explicit archival disposition of the legacy
