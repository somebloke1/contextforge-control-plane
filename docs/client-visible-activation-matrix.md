...
| Pi | Global TypeScript extension shim plus `.project/context_forge_state.json` metadata | No Codex-style hook banner is expected. Pi should inject project-init guidance through the hidden `before_agent_start` system-prompt context and activate ContextForge routes on `session_start`. | `cf_project_init_*`, `cf_contextforge_pi_readback`, guidance lookup, and `cf_contextforge_pi_validate` through the Pi shim. | Missing a Codex hook banner is expected for Pi. Missing `before_agent_start` guidance, `session_start` activation, shim tools, or guidance in an approved Pi session is the relevant failure. | Pi client Docker for development validation; host Pi global install/reload remains issue #3. |
| OpenCode | Project-local `opencode.json`, `.opencode/plugins/contextforge-project-init.js`, and container/local helper runtime | No Codex-style hook banner is expected. OpenCode should receive project-local plugin/helper behavior from its own `experimental.chat.system.transform` / `session.created` hook surface. | `opencode mcp list`, the project-local `contextforge-helper` entry, and `scripts/opencode_project_init_hook.py` behavior where enabled by the fixture. | Missing a Codex hook banner is expected for OpenCode. Missing the helper MCP entry, `experimental.chat.system.transform` context injection, or `session.created` hook behavior in an approved OpenCode session is the relevant failure. | OpenCode client Docker for development validation. |
...
- Claim `Pi active` only from Pi-visible shim/helper/guidance or validation
  readback from an ordinary Pi session that has not disabled session
  lifecycle events. Do not require a Codex-style hook banner for Pi.
