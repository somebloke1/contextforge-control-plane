...
def build_snapshot(payload: dict[str, Any], *, cwd: Path | None = None, now: str | None = None) -> dict[str, Any]:
    ...
    snapshot = {
        ...
        "session_id_present": session_id(payload) is not None,
        "repo_root": str(repo_root),
        "cwd": str(actual_cwd),
        "project_name": "ContextForge",
        "legacy_path_name": "context-portal",
        "mission": (
            "Execute the ContextForge roadmap as verified, issue-sliced, idempotent state transitions "
            "with no hidden technical, operational, documentation, coordination, or GitHub debt."
        ),
        "continuity_protocol": [
            ...
            "Complete a goal-maintenance/refinement pass before moving to the next roadmap loop.",
        ],
...
def persist_snapshot(snapshot: dict[str, Any], repo_root: Path) -> dict[str, str]:
    ...
    with lock_path.open("a", encoding="utf-8") as lock_handle:
        ...
        write_json_atomic(latest_json, snapshot)
        write_text_atomic(latest_md, render_markdown(snapshot))
        fcntl.flock(lock_handle.fileno(), fcntl.LOCK_UN)
    return {
        "event": str(event_path.relative_to(repo_root)),
        "latest_json": str(latest_json.relative_to(repo_root)),
        "latest_markdown": str(latest_md.relative_to(repo_root)),
    }


def additional_context(paths: dict[str, str]) -> str:
    latest = paths.get("latest_markdown", "run/codex-precompact-continuity/latest.md")
    return (
        f"ContextForge pre-compaction continuity snapshot refreshed at `{latest}`. "
        "Use it as additive continuity evidence after compaction; keep Codex's default compaction prompt authoritative. "
        "Preserve the roadmap conductor mission, issue-sliced GitHub topology, dirty-state protection, idempotency, "
        "ContextForge naming boundary, and the goal-maintenance step before the next loop."
    )
