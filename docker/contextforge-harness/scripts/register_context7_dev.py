#!/usr/bin/env python3
"""Register the dev-Docker Context7 transceiver with the harness gateway."""
...
DEFAULT_UPSTREAM_URL = "http://context7-transceiver:9203/mcp"
...
def gateway_body(upstream_url: str) -> dict[str, Any]:
    return {
        "name": GATEWAY_NAME,
        "url": upstream_url,
        "description": "Dev-Docker Context7 documentation MCP backend exposed through stock mcpgateway.translate.",
        "transport": "STREAMABLEHTTP",
        "tags": ["contextforge", "dev-docker", "context7", "local-backend"],
        "visibility": "public",
        "owner_email": OWNER,
        "gateway_mode": "cache",
    }
...
def server_body(tool_ids: list[str]) -> dict[str, Any]:
    return {
        "name": SERVER_NAME,
        "description": "Dev-Docker virtual server exposing the Context7 transceiver sidecar.",
        "associated_tools": tool_ids,
        "tags": ["contextforge", "dev-docker", "context7"],
        "owner_email": OWNER,
        "visibility": "public",
    }
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--upstream-url", default=os.environ.get("CONTEXT7_TRANSCEIVER_URL", DEFAULT_UPSTREAM_URL))
    return parser
