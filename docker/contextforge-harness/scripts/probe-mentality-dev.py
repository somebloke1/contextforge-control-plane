#!/usr/bin/env python3
"""Probe direct and ContextForge-virtual mentality dev-Docker MCP endpoints."""
...
import register_mentality_dev as registration


DEFAULT_DIRECT_URL = "http://127.0.0.1:9201/mcp"
...
def create_probe_token(base_url: str, admin_token: str, server_id: str) -> tuple[str, str]:
    response = registration.request_json(
        ...
        token=admin_token,
        body={
            "name": f"mentality-dev-docker-probe-{uuid4().hex[:12]}",
            "description": "Ephemeral dev harness token for virtual MCP probe.",
            "expires_in_days": 1,
            "scope": {
                "server_id": server_id,
                "permissions": PROBE_TOKEN_PERMISSIONS,
            },
            "tags": ["contextforge", "dev-docker", "probe", "ephemeral"],
        },
    )
...
def revoke_probe_token(base_url: str, admin_token: str, token_id: str) -> None:
    registration.request_json(
        ...
        f"/tokens/{token_id}",
        token=admin_token,
        body={"reason": "dev harness virtual MCP probe complete"},
    )
