#!/usr/bin/env python3
"""Register the dev-Docker mentality transceiver with the harness gateway."""
...
GATEWAY_NAME = "mentality-dev-docker"
SERVER_NAME = "mentality_dev_docker_server"
DEFAULT_GATEWAY_BASE = "http://127.0.0.1:4445"
DEFAULT_UPSTREAM_URL = "http://mentality-transceiver:9201/mcp"
...
def gateway_body(upstream_url: str) -> dict[str, Any]:
    return {
        ...
        "url": upstream_url,
        "description": (
            "Dev-Docker mentality governance MCP backend exposed through "
            "stock mcpgateway.translate on the reserved 9201 port."
        ),
        "transport": "STREAMABLEHTTP",
        "tags": ["contextforge", "dev-docker", "mentality", "local-backend"],
        "visibility": "public",
        "owner_email": OWNER,
        "gateway_mode": "cache",
    }
...
def server_body(tool_ids: list[str]) -> dict[str, Any]:
    return {
        "name": SERVER_NAME,
        "description": "Dev-Docker virtual server exposing the mentality transceiver sidecar.",
        "associated_tools": tool_ids,
        "tags": ["contextforge", "dev-docker", "mentality"],
        "owner_email": OWNER,
        "visibility": "public",
    }
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--upstream-url", default=os.environ.get("MENTALITY_TRANSCEIVER_URL", DEFAULT_UPSTREAM_URL))
    return parser
