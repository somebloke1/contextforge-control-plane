#!/usr/bin/env python3
"""Assemble Use Case 5f ssh-tmux lifecycle evidence for evaluator review."""
...
ISSUES = [247, 266, 259, 260, 263, 264, 270]
PR_NUMBER = 271
...
def render_evidence(
    *,
    ...
    lines = [
        "# Use Case 5f ssh-tmux Lifecycle Evidence",
        "",
        f"Timestamp: {timestamp}",
        "Use case: UC5f / issue #266 ssh-tmux session-scoped service lifecycle and apply proof",
...
def render_package(
    *,
    ...
    return "\n".join(
        [
            "# Evaluation Package: Use Case 5f ssh-tmux Lifecycle",
            "",
            "Use case: ssh-tmux session-scoped lifecycle and apply proof for #247 (#266).",
            ...
            "## Methodology",
            "",
            "This source-evidence package localizes the holistic SuperLoop for the ssh-tmux session-scoped lifecycle slice. The runner assembles current issue/PR bodies, source artifact summaries, focused test output, metadata, and structural verifier output. It does not score lifecycle adequacy.",
            "",
            "## Source Artifacts",
            "",
            "```json",
            ...
            "- session_scope_boundary: 15",
            "- source_boundary: 10",
            "- deterministic_boundary: 10",
            "",
            "Fatal failures: ssh-tmux matrix row missing or not routed to #266, Pi/OpenCode lifecycle tests failing, unsafe session boundary violation, global/client-home/trust/secret/live ContextForge mutation, session open/send/close/remote-file behavior, post-install probe leakage, #247/#269/remaining-service overclaim, or deterministic semantic scoring of prose.",
            "",
            "Required evaluator narrative: summarize issue context, inspect the ssh-tmux lifecycle evidence, explain whether #266 is satisfied for the source/apply slice, identify session-scope or overclaim risks, score each criterion, and return PASS or FAIL.",
        ]
    ) + "\n"
...
def main(argv: list[str] | None = None) -> int:
    ...
    output_root = harness_root / "evidence" / "use-case-5f" / "ssh-tmux-lifecycle"
    output_root.mkdir(parents=True, exist_ok=True)
    commands: list[dict[str, Any]] = []

    context = github_context(repo_root, commands)
    test_command = run(
        [sys.executable, "-m", "unittest", "tests.test_ssh_tmux_single_service_lifecycle", "-v"],
        cwd=repo_root,
    ...
    artifacts = {
        ...
        "source_test": "tests/test_ssh_tmux_single_service_lifecycle.py",
        "package": "docs/use-cases/use-case-5f-ssh-tmux/package.md",
        ...
        "ssh_tmux_manifest": "server-instances/ssh-tmux/instance.json",
    }
    metadata = {
    ...
    metadata_path = output_root / f"ssh-tmux-lifecycle-metadata-{timestamp}.json"
    evidence_path = output_root / f"ssh-tmux-lifecycle-use-case-5f-evidence-{timestamp}.md"
    package_path = output_root / f"ssh-tmux-lifecycle-evaluation-package-{timestamp}.md"
    verifier_path = output_root / f"ssh-tmux-lifecycle-verifier-{timestamp}.json"
    ...
    verifier_script = Path(__file__).with_name("verify-use-case-5f-ssh-tmux-lifecycle.py")
