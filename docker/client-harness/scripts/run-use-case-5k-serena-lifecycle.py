#!/usr/bin/env python3
"""Assemble Use Case 5k Serena lifecycle evidence for evaluator review."""
...
ISSUES = [247, 268, 267, 259, 260, 261, 262, 263, 264, 265, 266, 270]
PR_NUMBER = 271
...
def render_evidence(
    *,
    ...
    lines = [
        "# Use Case 5k Serena Lifecycle Evidence",
        "",
        f"Timestamp: {timestamp}",
        "Use case: UC5k / issue #268 Serena project-scoped service lifecycle and apply proof",
...
def render_package(
    *,
    ...
    return "\n".join(
        [
            "# Evaluation Package: Use Case 5k Serena Lifecycle",
            "",
            "Use case: Serena project-scoped lifecycle and apply proof for #247 (#268).",
            ...
            "## Methodology",
            "",
            "This source-evidence package localizes the holistic SuperLoop for the Serena project-scoped lifecycle slice. The runner assembles current issue/PR bodies, source artifact summaries, focused test output, metadata, and structural verifier output. It does not score lifecycle adequacy.",
            "",
            "## Source Artifacts",
            "",
            "```json",
            ...
            "- project_scoped_provisioning_boundary: 20",
            "- source_boundary: 10",
            "- deterministic_boundary: 15",
            "",
            "Fatal failures: Serena matrix row missing or not routed to #268, Pi/OpenCode lifecycle tests failing, skipped language-input stop, project-scoped provisioning not represented, global/client-home/trust/secret/live ContextForge mutation beyond the modeled provision helper, direct Serena runtime/LSP/indexing overclaim, #247/#269 overclaim, or deterministic semantic scoring of prose.",
            "",
            "Required evaluator narrative: summarize issue context, inspect the Serena lifecycle evidence, explain whether #268 is satisfied for the source/apply slice, identify project-scoped provisioning or overclaim risks, score each criterion, and return PASS or FAIL.",
        ]
    ) + "\n"
...
def main(argv: list[str] | None = None) -> int:
    ...
    output_root = harness_root / "evidence" / "use-case-5k" / "serena-lifecycle"
    output_root.mkdir(parents=True, exist_ok=True)
    commands: list[dict[str, Any]] = []

    context = github_context(repo_root, commands)
    test_command = run(
        [sys.executable, "-m", "unittest", "tests.test_serena_single_service_lifecycle", "-v"],
        cwd=repo_root,
    ...
    artifacts = {
        ...
        "source_test": "tests/test_serena_single_service_lifecycle.py",
        "package": "docs/use-cases/use-case-5k-serena/package.md",
        ...
        "serena_manifest": "server-instances/serena-cf-controlplane-d46fe58a2a20/instance.json",
        "serena_manager": "scripts/manage_serena_project_instance.py",
    }
    metadata = {
    ...
    metadata_path = output_root / f"serena-lifecycle-metadata-{timestamp}.json"
    evidence_path = output_root / f"serena-lifecycle-use-case-5k-evidence-{timestamp}.md"
    package_path = output_root / f"serena-lifecycle-evaluation-package-{timestamp}.md"
    verifier_path = output_root / f"serena-lifecycle-verifier-{timestamp}.json"
    ...
    verifier_script = Path(__file__).with_name("verify-use-case-5k-serena-lifecycle.py")
