#!/usr/bin/env python3
"""Assemble Use Case 5c Context7 lifecycle evidence for evaluator review."""
...
def render_evidence(
    *,
    ...
    lines = [
        "# Use Case 5c Context7 Lifecycle Evidence",
        "",
        f"Timestamp: {timestamp}",
        "Use case: UC5c / issue #260 Context7 single-service lifecycle and apply proof",
...
def render_package(
    *,
    ...
    return "\n".join(
        [
            "# Evaluation Package: Use Case 5c Context7 Lifecycle",
            "",
            "Use case: Context7 single-service lifecycle and apply proof for #247 (#260).",
            ...
            "## Methodology",
            "",
            "This source-evidence package localizes the holistic SuperLoop for the Context7 single-service lifecycle slice. The runner assembles current issue/PR bodies, source artifact summaries, focused test output, metadata, and structural verifier output. It does not score lifecycle adequacy.",
            "",
            "## Source Artifacts",
            "",
            "```json",
            ...
            "- source_boundary: 15",
            "- install_only_boundary: 10",
            "- deterministic_boundary: 10",
            "",
            "Fatal failures: Context7 matrix row missing or not routed to #260, Pi/OpenCode lifecycle tests failing, global/client-home/trust/secret/live ContextForge mutation, post-install validation/probe leakage, #247/#269/#261-#268 overclaim, or deterministic semantic scoring of prose.",
            "",
            "Required evaluator narrative: summarize issue context, inspect the Context7 lifecycle evidence, explain whether #260 is satisfied for the source/apply slice, identify overclaim risks, score each criterion, and return PASS or FAIL.",
        ]
    ) + "\n"
...
def main(argv: list[str] | None = None) -> int:
    ...
    output_root = harness_root / "evidence" / "use-case-5c" / "context7-lifecycle"
    output_root.mkdir(parents=True, exist_ok=True)
    commands: list[dict[str, Any]] = []

    context = github_context(repo_root, commands)
    test_command = run(
        [sys.executable, "-m", "unittest", "tests.test_context7_single_service_lifecycle", "-v"],
        cwd=repo_root,
    ...
    artifacts = {
        ...
        "source_test": "tests/test_context7_single_service_lifecycle.py",
        "package": "docs/use-cases/use-case-5c/package.md",
    ...
    metadata_path = output_root / f"context7-lifecycle-metadata-{timestamp}.json"
    metadata_path.write_text(json.dumps(metadata, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    evidence_path = output_root / f"context7-lifecycle-use-case-5c-evidence-{timestamp}.md"
    evidence_path.write_text(render_evidence(timestamp=timestamp, commands=commands, metadata=metadata), encoding="utf-8")

    verifier_result = run(
        [
            sys.executable,
            str(harness_root / "scripts" / "verify-use-case-5c-context7-lifecycle.py"),
            "--evidence",
            str(evidence_path),
            "--metadata",
            str(metadata_path),
    ...
    verifier_path = output_root / f"context7-lifecycle-verifier-{timestamp}.json"
    ...
    package_path = output_root / f"context7-lifecycle-evaluation-package-{timestamp}.md"
