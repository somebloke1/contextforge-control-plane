...
def main(argv: list[str] | None = None) -> int:
    ...
    session_id = f"uc2-{args.client}-{timestamp}" if args.client == "pi" else ""
    commands: list[dict[str, Any]] = []

    reset = run(
        [sys.executable, str(harness_root / "scripts" / "reset-client-harness-state.py"), "--client", args.client, "--reset-home-volume", "--evidence-dir", "docker/client-harness/evidence/use-case-2/prior"],
        cwd=repo_root,
        timeout=120,
        commands=commands,
    )
