...
PY
  ...
  pi --no-session --no-tools --no-context-files --provider local-llama-qwen --model qwen3.6-a3b -p "Reply with exactly: pi-qwen-ok"
' | tee evidence/pi-agent-smoke.txt

echo "== OpenCode agent smoke =="
docker compose -f compose.yml run --rm opencode bash -lc '
  set -euo pipefail
  opencode run --model "llama.cpp/${LOCAL_LLAMA_MODEL}" --agent build --format default "Reply with exactly: opencode-qwen-ok"
