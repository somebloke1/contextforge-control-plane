#!/usr/bin/env python3
"""Assemble Use Case 5j web-search lifecycle evidence for evaluator review."""
...
ISSUES = [247, 267, 259, 260, 261, 262, 263, 264, 265, 266, 270]
PR_NUMBER = 271
...
def render_evidence(
    *,
    ...
    lines = [
        "# Use Case 5j web-search Lifecycle Evidence",
        "",
        f"Timestamp: {timestamp}",
        "Use case: UC5j / issue #267 web-search credential-scoped service lifecycle and apply proof",
...
def render_package(
    *,
    ...
    return "\n".join(
        [
            "# Evaluation Package: Use Case 5j web-search Lifecycle",
            "",
            "Use case: web-search credential-scoped lifecycle and apply proof for #247 (#267).",
            ...
            "## Methodology",
            "",
            "This source-evidence package localizes the holistic SuperLoop for the web-search credential-scoped lifecycle slice. The runner assembles current issue/PR bodies, source artifact summaries, focused test output, metadata, and structural verifier output. It does not score lifecycle adequacy.",
            "",
            "## Source Artifacts",
            "",
            "```json",
            ...
            "",
            "Fatal failures: web-search matrix row missing or not routed to #267, Pi/OpenCode lifecycle tests failing, credential boundary violation, global/client-home/trust/secret/live ContextForge mutation, provider credential writes or exposure, built-in web-search substitution, post-install provider/search/fetch probe leakage, #247/#269/#268 overclaim, or deterministic semantic scoring of prose.",
            "",
            "Required evaluator narrative: summarize issue context, inspect the web-search lifecycle evidence, explain whether #267 is satisfied for the source/apply slice, identify credential or overclaim risks, score each criterion, and return PASS or FAIL.",
        ]
    ) + "\n"
...
def main(argv: list[str] | None = None) -> int:
    ...
    output_root = harness_root / "evidence" / "use-case-5j" / "web-search-lifecycle"
    output_root.mkdir(parents=True, exist_ok=True)
    commands: list[dict[str, Any]] = []

    context = github_context(repo_root, commands)
    test_command = run(
        [sys.executable, "-m", "unittest", "tests.test_web_search_single_service_lifecycle", "-v"],
        cwd=repo_root,
    ...
    artifacts = {
        ...
        "source_test": "tests/test_web_search_single_service_lifecycle.py",
        "package": "docs/use-cases/use-case-5j-web-search/package.md",
        ...
        "web_search_manifest": "server-instances/web-search/instance.json",
    }
    metadata = {
    ...
    metadata_path = output_root / f"web-search-lifecycle-metadata-{timestamp}.json"
    evidence_path = output_root / f"web-search-lifecycle-use-case-5j-evidence-{timestamp}.md"
    package_path = output_root / f"web-search-lifecycle-evaluation-package-{timestamp}.md"
    verifier_path = output_root / f"web-search-lifecycle-verifier-{timestamp}.json"
    ...
    verifier_script = Path(__file__).with_name("verify-use-case-5j-web-search-lifecycle.py")
