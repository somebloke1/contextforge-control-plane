#!/usr/bin/env python3
"""Assemble Use Case 5g Playwright lifecycle evidence for evaluator review."""
...
ISSUES = [247, 265, 259, 260, 263, 264, 266, 270]
PR_NUMBER = 271
...
def render_evidence(
    *,
    ...
    lines = [
        "# Use Case 5g Playwright Lifecycle Evidence",
        "",
        f"Timestamp: {timestamp}",
        "Use case: UC5g / issue #265 Playwright session-scoped service lifecycle and apply proof",
...
def render_package(
    *,
    ...
    return "\n".join(
        [
            "# Evaluation Package: Use Case 5g Playwright Lifecycle",
            "",
            "Use case: Playwright session-scoped lifecycle and apply proof for #247 (#265).",
            ...
            "## Methodology",
            "",
            "This source-evidence package localizes the holistic SuperLoop for the Playwright session-scoped lifecycle slice. The runner assembles current issue/PR bodies, source artifact summaries, focused test output, metadata, and structural verifier output. It does not score lifecycle adequacy.",
            "",
            "## Source Artifacts",
            "",
            "```json",
            ...
            "- controlled_fixture_boundary: 15",
            "- source_boundary: 10",
            "- deterministic_boundary: 10",
            "",
            "Fatal failures: Playwright matrix row missing or not routed to #265, Pi/OpenCode lifecycle tests failing, arbitrary browsing/screenshot/unsafe-code boundary violation, global/client-home/trust/secret/live ContextForge mutation, host browser/global config mutation, post-install probe leakage, #247/#269/remaining-service overclaim, or deterministic semantic scoring of prose.",
            "",
            "Required evaluator narrative: summarize issue context, inspect the Playwright lifecycle evidence, explain whether #265 is satisfied for the source/apply slice, identify fixture-page or overclaim risks, score each criterion, and return PASS or FAIL.",
        ]
    ) + "\n"
...
def main(argv: list[str] | None = None) -> int:
    ...
    output_root = harness_root / "evidence" / "use-case-5g" / "playwright-lifecycle"
    output_root.mkdir(parents=True, exist_ok=True)
    commands: list[dict[str, Any]] = []

    context = github_context(repo_root, commands)
    test_command = run(
        [sys.executable, "-m", "unittest", "tests.test_playwright_single_service_lifecycle", "-v"],
        cwd=repo_root,
    ...
    artifacts = {
        ...
        "source_test": "tests/test_playwright_single_service_lifecycle.py",
        "package": "docs/use-cases/use-case-5g-playwright/package.md",
        ...
        "playwright_manifest": "server-instances/playwright/instance.json",
    }
    metadata = {
    ...
    metadata_path = output_root / f"playwright-lifecycle-metadata-{timestamp}.json"
    evidence_path = output_root / f"playwright-lifecycle-use-case-5g-evidence-{timestamp}.md"
    package_path = output_root / f"playwright-lifecycle-evaluation-package-{timestamp}.md"
    verifier_path = output_root / f"playwright-lifecycle-verifier-{timestamp}.json"
    ...
    verifier_script = Path(__file__).with_name("verify-use-case-5g-playwright-lifecycle.py")
