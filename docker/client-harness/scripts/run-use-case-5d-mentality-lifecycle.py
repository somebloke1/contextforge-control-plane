#!/usr/bin/env python3
"""Assemble Use Case 5d mentality lifecycle evidence for evaluator review."""
...
def render_evidence(
    *,
    ...
    lines = [
        "# Use Case 5d Mentality Lifecycle Evidence",
        "",
        f"Timestamp: {timestamp}",
        "Use case: UC5d / issue #263 mentality repo-local/static service lifecycle and apply proof",
...
def render_package(
    *,
    ...
    return "\n".join(
        [
            "# Evaluation Package: Use Case 5d Mentality Lifecycle",
            "",
            "Use case: mentality repo-local/static lifecycle and apply proof for #247 (#263).",
            ...
            "## Methodology",
            "",
            "This source-evidence package localizes the holistic SuperLoop for the mentality repo-local/static lifecycle slice. The runner assembles current issue/PR bodies, source artifact summaries, focused test output, metadata, and structural verifier output. It does not score lifecycle adequacy.",
            "",
            "## Source Artifacts",
            "",
            "```json",
            ...
            "- governance_read_only_boundary: 15",
            "- source_boundary: 10",
            "- deterministic_boundary: 10",
            "",
            "Fatal failures: mentality matrix row missing or not routed to #263, Pi/OpenCode lifecycle tests failing, governance ledger mutation, global/client-home/trust/secret/live ContextForge mutation, post-install probe leakage, #247/#269/remaining-service overclaim, or deterministic semantic scoring of prose.",
            "",
            "Required evaluator narrative: summarize issue context, inspect the mentality lifecycle evidence, explain whether #263 is satisfied for the source/apply slice, identify governance read-only or overclaim risks, score each criterion, and return PASS or FAIL.",
        ]
    ) + "\n"
...
def main(argv: list[str] | None = None) -> int:
    ...
    output_root = harness_root / "evidence" / "use-case-5d" / "mentality-lifecycle"
    output_root.mkdir(parents=True, exist_ok=True)
    commands: list[dict[str, Any]] = []

    context = github_context(repo_root, commands)
    test_command = run(
        [sys.executable, "-m", "unittest", "tests.test_mentality_single_service_lifecycle", "-v"],
        cwd=repo_root,
    ...
    artifacts = {
        ...
        "source_test": "tests/test_mentality_single_service_lifecycle.py",
        "package": "docs/use-cases/use-case-5d-mentality/package.md",
        ...
        "governance_mcp": "scripts/governance_mcp.py",
        "mentality_manifest": "server-instances/mentality/instance.json",
    }
    metadata = {
        "use_case": "use-case-5d-mentality",
    ...
    metadata_path = output_root / f"mentality-lifecycle-metadata-{timestamp}.json"
    metadata_path.write_text(json.dumps(metadata, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    evidence_path = output_root / f"mentality-lifecycle-use-case-5d-evidence-{timestamp}.md"
    evidence_path.write_text(render_evidence(timestamp=timestamp, commands=commands, metadata=metadata), encoding="utf-8")

    verifier_result = run(
        [
            sys.executable,
            str(harness_root / "scripts" / "verify-use-case-5d-mentality-lifecycle.py"),
            "--evidence",
            str(evidence_path),
            "--metadata",
            str(metadata_path),
    ...
    verifier_path = output_root / f"mentality-lifecycle-verifier-{timestamp}.json"
    ...
    package_path = output_root / f"mentality-lifecycle-evaluation-package-{timestamp}.md"
