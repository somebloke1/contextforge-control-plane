#!/usr/bin/env python3
"""Run Use Case 3 through a real client CLI and package evaluation evidence."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.timeout < 90:
        raise SystemExit("--timeout must be at least 90 seconds for Use Case 3 dialogue evaluation")

    repo_root = Path(__file__).resolve().parents[3]
    harness_root = repo_root / "docker" / "client-harness"
    lock_file = acquire_harness_lock(harness_root)
    timestamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    output_root = harness_root / "evidence" / "use-case-3" / args.client
    output_root.mkdir(parents=True, exist_ok=True)
    setup_container = f"cf-uc3-{args.client}-setup-{timestamp.lower().replace('z', '')}"
    container = f"cf-uc3-{args.client}-runner-{timestamp.lower().replace('z', '')}"
    session_id = f"uc3-{args.client}-{timestamp}" if args.client == "pi" else ""
    commands: list[dict[str, Any]] = []

    reset = run(
        [sys.executable, str(harness_root / "scripts" / "reset-client-harness-state.py"), "--client", args.client, "--reset-home-volume", "--evidence-dir", "docker/client-harness/evidence/use-case-3/prior"],
        cwd=repo_root,
        timeout=120,
        commands=commands,
    )
    ...
    combined_path = output_root / f"{args.client}-use-case-3-evidence-{timestamp}.md"
    ...
    verifier = run(
        [
            sys.executable,
            str(harness_root / "scripts" / "verify-use-case-3-e2e-evidence.py"),
            "--client",
            args.client,
            "--evidence",
            str(combined_path),
...
def initialized_fixture_command(client: str) -> str:
    approval = "/home/agent/.local/state/contextforge-client-harness-runtime/project-init/uc3-approval.json"
...
def render_combined(**kwargs: Any) -> str:
    lines = [
        f"# Use Case 3 {kwargs['client']} Dialogue Evidence",
        "",
...
def render_package(client: str, combined_path: Path, verifier_path: Path, verifier_json: Any, generation_report: dict[str, Any]) -> str:
    return "\n".join(
        [
            f"# Evaluation Package: {client} Use Case 3",
            "",
            "Use case: Discover available project capabilities from initialized ContextForge/project authority.",
            ...
            "Required narrative: setup, initialized fixture, prompt, visible capability summary, capability classes, provenance, non-actions, score, verdict.",
        ]
    )
