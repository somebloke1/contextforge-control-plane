#!/usr/bin/env python3
"""Assemble Use Case 5b readiness-matrix evidence for evaluator review."""
...
ISSUES = [247, 259, 270, *range(260, 269)]
PR_NUMBER = 271
...
def render_evidence(
    *,
    ...
    lines = [
        "# Use Case 5b Readiness Matrix Evidence",
        "",
        f"Timestamp: {timestamp}",
        "Use case: UC5b / issue #259 helper-offered service readiness matrix",
...
def render_package(
    *,
    ...
    return "\n".join(
        [
            "# Evaluation Package: Use Case 5b Readiness Matrix",
            "",
            "Use case: Helper-offered service readiness matrix for #247 (#259).",
            f"Combined evidence: `{evidence_path}`",
            f"Metadata: `{metadata_path}`",
            f"Verifier: `{verifier_path}`",
            f"Deterministic verifier ok: `{bool(verifier_json.get('ok'))}`",
            "Deterministic verifier scope: `structured source artifacts, command facts, JSON shape; not semantic readiness acceptance`",
            "Acceptance status: `requires_agent_evaluator_scoring`",
            "",
            "## Methodology",
            "",
            "This source-evidence package localizes the holistic SuperLoop for a non-dialogue readiness-matrix slice. The runner assembles current issue/PR bodies, source artifact summaries, focused test output, metadata, and structural verifier output. It does not score readiness adequacy.",
            "",
            "## Source Artifacts",
            "",
            "```json",
            ...
            "- taxonomy_consumed: 10",
            "- service_coverage: 20",
            "- per_service_fields: 20",
            "- false_readiness_prevention: 20",
            "- current_slice_boundary: 10",
            "- deterministic_boundary: 10",
            "",
            "Fatal failures: missing required service, `None` treated as service readiness, matrix taxonomy mismatch, missing per-service owner/evidence path, all-services readiness overclaim, or deterministic semantic scoring of prose.",
            "",
            "Required evaluator narrative: summarize issue context, inspect the matrix/taxonomy artifacts, explain whether #260-#268 and #247 can consume the matrix safely, identify false-readiness risks, score each criterion, and return PASS or FAIL.",
        ]
    ) + "\n"
...
def main(argv: list[str] | None = None) -> int:
    ...
    output_root = harness_root / "evidence" / "use-case-5b" / "readiness-matrix"
    output_root.mkdir(parents=True, exist_ok=True)
    commands: list[dict[str, Any]] = []

    context = github_context(repo_root, commands)
    test_command = run(
        [sys.executable, "-m", "unittest", "tests.test_service_readiness_matrix", "-v"],
        cwd=repo_root,
    ...
    artifacts = {
        ...
        "package": "docs/use-cases/use-case-5b/package.md",
    }
    metadata = {
    ...
    metadata_path = output_root / f"readiness-matrix-metadata-{timestamp}.json"
    metadata_path.write_text(json.dumps(metadata, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    evidence_path = output_root / f"readiness-matrix-use-case-5b-evidence-{timestamp}.md"
    evidence_path.write_text(render_evidence(timestamp=timestamp, commands=commands, metadata=metadata), encoding="utf-8")

    verifier_result = run(
        [
            sys.executable,
            str(harness_root / "scripts" / "verify-use-case-5b-readiness-matrix.py"),
            "--evidence",
            str(evidence_path),
            "--metadata",
            str(metadata_path),
    ...
    verifier_path = output_root / f"readiness-matrix-verifier-{timestamp}.json"
    ...
    package_path = output_root / f"readiness-matrix-evaluation-package-{timestamp}.md"
