#!/usr/bin/env python3
"""Assemble Use Case 5h exa-search lifecycle evidence for evaluator review."""
...
ISSUES = [247, 261, 259, 260, 263, 264, 265, 266, 270]
PR_NUMBER = 271
...
def render_evidence(
    *,
    ...
    lines = [
        "# Use Case 5h exa-search Lifecycle Evidence",
        "",
        f"Timestamp: {timestamp}",
        "Use case: UC5h / issue #261 exa-search credential-scoped service lifecycle and apply proof",
...
def render_package(
    *,
    ...
    return "\n".join(
        [
            "# Evaluation Package: Use Case 5h exa-search Lifecycle",
            "",
            "Use case: exa-search credential-scoped lifecycle and apply proof for #247 (#261).",
            ...
            "## Methodology",
            "",
            "This source-evidence package localizes the holistic SuperLoop for the exa-search credential-scoped lifecycle slice. The runner assembles current issue/PR bodies, source artifact summaries, focused test output, metadata, and structural verifier output. It does not score lifecycle adequacy.",
            "",
            "## Source Artifacts",
            "",
            "```json",
            ...
            "- pi_lifecycle_apply: 15",
            "- opencode_lifecycle_apply: 15",
            "- credential_boundary: 20",
            "- source_boundary: 10",
            "- deterministic_boundary: 15",
            "",
            "Fatal failures: exa-search matrix row missing or not routed to #261, Pi/OpenCode lifecycle tests failing, credential boundary violation, global/client-home/trust/secret/live ContextForge mutation, provider credential writes or exposure, post-install provider/search probe leakage, #247/#269/remaining-service overclaim, or deterministic semantic scoring of prose.",
            "",
            "Required evaluator narrative: summarize issue context, inspect the exa-search lifecycle evidence, explain whether #261 is satisfied for the source/apply slice, identify credential or overclaim risks, score each criterion, and return PASS or FAIL.",
        ]
    ) + "\n"
...
def main(argv: list[str] | None = None) -> int:
    ...
    output_root = harness_root / "evidence" / "use-case-5h" / "exa-search-lifecycle"
    output_root.mkdir(parents=True, exist_ok=True)
    commands: list[dict[str, Any]] = []

    context = github_context(repo_root, commands)
    test_command = run(
        [sys.executable, "-m", "unittest", "tests.test_exa_search_single_service_lifecycle", "-v"],
        cwd=repo_root,
    ...
    artifacts = {
        ...
        "source_test": "tests/test_exa_search_single_service_lifecycle.py",
        "package": "docs/use-cases/use-case-5h-exa-search/package.md",
        ...
        "exa_search_manifest": "server-instances/exa-search/instance.json",
    }
    metadata = {
    ...
    metadata_path = output_root / f"exa-search-lifecycle-metadata-{timestamp}.json"
    evidence_path = output_root / f"exa-search-lifecycle-use-case-5h-evidence-{timestamp}.md"
    package_path = output_root / f"exa-search-lifecycle-evaluation-package-{timestamp}.md"
    verifier_path = output_root / f"exa-search-lifecycle-verifier-{timestamp}.json"
    ...
    verifier_script = Path(__file__).with_name("verify-use-case-5h-exa-search-lifecycle.py")
    verifier_command = run(
        [sys.executable, str(verifier_script), "--evidence", str(evidence_path), "--metadata", str(metadata_path)],
        cwd=repo_root,
        timeout=args.timeout,
    )
    ...
    evidence_path.write_text(
        render_evidence(
            ...
            verifier_json=verifier_json if isinstance(verifier_json, dict) else {"raw": verifier_json},
        ),
        encoding="utf-8",
    )
    ...
    ok = test_command["returncode"] == 0 and isinstance(verifier_json, dict) and verifier_json.get("ok") is True
    print(
        json.dumps(
            {
                "ok": ok,
                ...
                "verifier_failures": verifier_json.get("failures") if isinstance(verifier_json, dict) else ["verifier_json_parse_failed"],
