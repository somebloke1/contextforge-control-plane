#!/usr/bin/env python3
"""Assemble Use Case 5i GitHub lifecycle evidence for evaluator review."""
...
def render_evidence(
    *,
    ...
    lines = [
        "# Use Case 5i GitHub Lifecycle Evidence",
        "",
        f"Timestamp: {timestamp}",
        "Use case: UC5i / issue #262 GitHub credential-scoped service lifecycle and apply proof",
...
def render_package(
    *,
    ...
    return "\n".join(
        [
            "# Evaluation Package: Use Case 5i GitHub Lifecycle",
            "",
            "Use case: GitHub credential-scoped lifecycle and apply proof for #247 (#262).",
            ...
            "## Methodology",
            "",
            "This source-evidence package localizes the holistic SuperLoop for the GitHub credential-scoped lifecycle slice. The runner assembles current issue/PR bodies, source artifact summaries, focused test output, metadata, and structural verifier output. It does not score lifecycle adequacy.",
            "",
            "## Source Artifacts",
            "",
            "```json",
            ...
            "",
            "Fatal failures: GitHub matrix row missing or not routed to #262, Pi/OpenCode lifecycle tests failing, credential boundary violation, global/client-home/trust/secret/live ContextForge mutation, token writes or exposure, post-install GitHub API/tool probe leakage, issue/PR/comment/label/branch/repository mutation, #247/#269/remaining-service overclaim, or deterministic semantic scoring of prose.",
            "",
            "Required evaluator narrative: summarize issue context, inspect the GitHub lifecycle evidence, explain whether #262 is satisfied for the source/apply slice, identify credential/mutation/overclaim risks, score each criterion, and return PASS or FAIL.",
        ]
    ) + "\n"
...
def main(argv: list[str] | None = None) -> int:
    ...
    output_root = harness_root / "evidence" / "use-case-5i" / "github-lifecycle"
    output_root.mkdir(parents=True, exist_ok=True)
    commands: list[dict[str, Any]] = []

    context = github_context(repo_root, commands)
    test_command = run(
        [sys.executable, "-m", "unittest", "tests.test_github_single_service_lifecycle", "-v"],
        cwd=repo_root,
    ...
    artifacts = {
        ...
        "source_test": "tests/test_github_single_service_lifecycle.py",
        "package": "docs/use-cases/use-case-5i-github/package.md",
        ...
        "project_init_common": "scripts/project_init_common.py",
        "github_manifest": "server-instances/github/instance.json",
    }
    metadata = {
    ...
    package_path = output_root / f"github-lifecycle-evaluation-package-{timestamp}.md"
    ...
    verifier_command = run(
        [
            sys.executable,
            str(harness_root / "scripts" / "verify-use-case-5i-github-lifecycle.py"),
            "--evidence",
            str(evidence_path),
            "--metadata",
            str(metadata_path),
    ...
    evidence_path.write_text(
        render_evidence(
            timestamp=timestamp,
            commands=commands + [verifier_command],
            metadata=metadata,
    ...
    result = {
        ...
        "verifier_failures": verifier_json.get("failures") if isinstance(verifier_json, dict) else ["invalid_verifier_json"],
        "ok": test_command["returncode"] == 0 and verifier_command["returncode"] == 0 and bool(isinstance(verifier_json, dict) and verifier_json.get("ok")),
