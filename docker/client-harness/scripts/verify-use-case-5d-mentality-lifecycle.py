#!/usr/bin/env python3
"""Verify Use Case 5d mentality lifecycle evidence structure."""
...
REQUIRED_ISSUE_KEYS = {"issue_247", "issue_263", "issue_259", "issue_260", "issue_270", "pr_271"}
...
def main(argv: list[str] | None = None) -> int:
    ...
    mentality_rows = [item for item in services if item.get("service") == "mentality"]
    mentality = mentality_rows[0] if len(mentality_rows) == 1 else {}
    taxonomy_types = {item.get("id") for item in taxonomy.get("types", [])}
    non_actions = set(mentality.get("blockers_or_non_actions") or [])
    ...
    check(metadata.get("use_case") == "use-case-5d-mentality", checks, failures, "metadata_use_case")
    ...
    check(len(mentality_rows) == 1, checks, failures, "single_mentality_matrix_row")
    check(mentality.get("owner_issue") == 263, checks, failures, "mentality_owner_issue")
    check(mentality.get("taxonomy_type") == "repo_local_static", checks, failures, "mentality_taxonomy_type")
    check(mentality.get("taxonomy_type") in taxonomy_types, checks, failures, "mentality_taxonomy_type_known")
    check(mentality.get("service_binding") == "mentality:static_repo_local", checks, failures, "mentality_binding")
    check(mentality.get("safe_probe_status") == "known_safe_probe", checks, failures, "known_safe_probe_status")
    check(
        {
            "no DECISIONS.md, ABEYANT_INTENTIONS.md, or OPEN_QUESTIONS.md mutation",
            "do not treat local file reads as target-client proof",
        }
        <= non_actions,
        checks,
        failures,
        "governance_non_actions_present",
    )
    ...
    check(
        "test_mentality_pi_lifecycle_writes_only_project_state_and_preserves_ledgers" in test_output,
        checks,
        failures,
        "pi_lifecycle_test_executed",
    )
    check(
        "test_mentality_opencode_lifecycle_writes_project_config_state_and_preserves_ledgers" in test_output,
        checks,
        failures,
    ...
    result = {
        ...
        "deterministic_boundary": (
            ...
            "matrix routing for mentality, issue-context shape, and focused test command status. It "
        ...
        "semantic_criteria": [
            {
                "id": "mentality_satisfies_issue_263",
                "question": "Does the source evidence satisfy #263's mentality lifecycle/apply proof purpose?"
            },
            {
                "id": "governance_read_only_boundary",
                "question": "Does the evidence preserve read-only governance behavior and ledger immutability?"
            },
            {
